...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        for blk, width, height in zip(feat, width_list, height_list):
            ...
            for row in range(height):
                ...
                for col in range(width):
                    tile = blk[col + row * width].cpu().detach().numpy()
                    big_blk_col = np.hstack((big_blk_col, tile))
                big_blk = np.vstack((big_blk, big_blk_col))
            tile_big = np.vstack((tile_big, big_blk))
        tile_big = tile_big.astype(np.uint16) #ccr added: features map spliced
        #cv2.imwrite('/media/data/ccr/trainft1.png', tile_big)
        ###############################################################################################################20220519
        # 20220516 ccr added end
        ###############################################################################################################
        # 20220517 ccr added start
        pyramid = {}
        png = tile_big.astype(np.float32)
        ...
        print(pyramid['p5'].size(),'-----------------------------------------------pyramid')
        features = pyramid
        # 20220517 ccr added end
        print(features['p5'].size(),'----------------------------------------------------------------------GeneralizedRCNN features')
    ...
    def inference(
        ...
        images = self.preprocess_image(batched_inputs)
        features = self.backbone(images.tensor)
        print(features['p5'].size(),'----------------------------------------------------------------------GeneralizedRCNN inference features')
